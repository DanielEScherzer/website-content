...
for me to reflect back on my PHP contributions in 2025. It was a pretty big
year for me in terms of contributing to the php-src repo.
...
Since my cleanup changes have generally been aimed at not having any visible
impact on the results, just improving the script, they probably were not that
useful (after all, the script functioned the same afterwards) but it was still
fun to work on. That said, I also made a number of improvements to the script
that *did* change the output in (what I hope was) a positive way, most of them
relating to processing attributes on internal structures:

* After support for attributes on compile-time constants was merged (details
...
deprecated in, by adding known strings for each of the applicable versions the
registration was simplified to no longer need to look up the strings each time
...
improvement in compilation times if the conditions needed to be evaluated fewer
times.
...
in the directory, they cannot be easily enumerated in the online interface.
...
branches of PHP. This meant that, once my change to add myself as a code owner
for reflection in PHP 8.3 was merged ([#18138][gh-18138]), I then needed to
...
Some of the constants were not handled as part of that patch, because of how
the constants were declared or used. I sent additional patches for
...
The `never` parameters RFC was declined by the community, while the rest were
accepted. You can read more about the `never` parameter types RFC and support
for `final` in constructor property promotion in my blog post from June,
...
contributor came when I was chosen as one of the release managers for PHP 8.5.
While I started this role in 2025, it will continue until the end of 2029, when
PHP 8.5 reaches its end of life. I'm planning to write up a separate blog post
about my experience so far.

## Looking Ahead

So far in 2026, my merged PHP contributions have only been related to my work as
a release manager. But, I've already started work on a few RFCs to hopefully
include in PHP 8.6. I plan to remain involved as a contributor at least until
the end of 2029 when my role as a release manager ends, so I'm not going
anywhere.
